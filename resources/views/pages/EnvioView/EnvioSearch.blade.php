...
<form>
    <div class="form-horizontal">
        ...
        <div class="col-sm-6">
            <input type="text" id="envid" class="form-control" value="{{$envio->ENVID}}" name="envid" readonly>
            </div>
        </div>
        <div class="form-group">
        <label class="control-label col-sm-3" for="envdestino">Destino</label>
        <div class="col-sm-6">
            <input type="text" class="form-control" id="envdestino" value="{{$envio->ENVDESTINO}}" name="envdestino">
            </div>
        </div>
        ...
        <div class="col-sm-6">
            <input type="text" class="form-control" id="envprecio" value="{{$envio->ENVPRECIO}}" name="envprecio">
            </div>
        </div>
        @endforeach
...
<table class="table order-list" id ="tabla">
    ...
    <tbody id="cuerpo">
    @foreach($envpaquetes as $envpaq)
        <tr>
            
            <td><form action="{{action('EnvioController@destroy', $envpaq->PAQID)}}" method="post">
            @csrf
            <input name="_method" type="hidden" value="DELETE">
            <button class="btn btn-danger" type="submit"><span class="oi oi-trash"></span></button>
            
          </form></td>
            <td contenteditable="false">{{$envpaq->PAQID}}</td>
            <td contenteditable="false">{{$envpaq->PAQDESCRIPCION}}</td>
            <td contenteditable="false">{{$envpaq->PAQFECHAINGRESO}}</td>
  ...
  <div class="modal-dialog" role="document">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <div class="form-group">
        <label class="control-label col-sm-3" for="paqfechaingreso">Fecha Ingreso</label>
        <div class="col-sm-6">
        <input type="text" class="form-control" form="form1" id="paqfechaingreso"  name="paqfechaingreso">   
            </div>
        </div>
